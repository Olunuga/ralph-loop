...
- `run/SKILL.md` — `/ralph-loop:run` orchestrator. Creates worktree, runs plan, monitors build loop, does post-mortem, cleans up. Auto-detects plan-slc vs plan-work based on AUDIENCE_JTBD.md. Has critical rules (don't edit files, use iteration_context.md, blast radius policy).
...
**Key project file: `ralph/AUDIENCE_JTBD.md`**
Permanent audience context generated by `/ralph-loop:req-slc`. Never archived — spans releases. Its presence triggers `plan-slc` mode instead of `plan-work` when running `/ralph-loop:run`.

**`schemas/ralph-bridge/` : OpenSpec community schema (the bridge)**
- `schema.yaml` : five artifacts (`proposal`, `specs`, `design`, `tasks`, `gate-report`). The `design` and `tasks` instructions name `ralph/gate_context.md` and the gate directories as required reads, so planning does not propose work a gate rejects. The `apply` instruction judges on `post minus baseline`.
- `bundle.json` : `minOpenspecVersion`. `scripts/schema_stamp.sh require-min` enforces it.
- `templates/` : one Markdown template per artifact.
- Installed into a project at `openspec/schemas/ralph-bridge/` by `/ralph-loop:init --openspec`, which also writes `.stamp.json` there.

**`hooks/` : git hooks and the Claude Stop hook**
- `pre-commit` : runs the `fast` static tier. Blocks the commit on failure.
- `pre-push` : runs the `precise` static tier, then the LLM gates. Static runs first, so a deterministic failure stops the push before any model call.
- `stop_gate_check.sh` : opt-in, `fast` tier only, never LLM.
- All three exit 0 with a notice when the gate engine cannot run. A missing pipeline never blocks a commit or push.
...
A release sliced before that step existed is repaired by `slice` Step 1b. It runs the same
three-part test over the changes already in the newest release record, inserts a row for each
missing foundation ahead of every cell, renumbers below them, and creates no new release.
`--add-theme` narrows it to the theme alone and exists only for that older case. That is the one case that edits an existing release record. A user
...
variables, each defaulting to the legacy location:

- `RALPH_BRIEF_DIR` : where the build agent reads the brief. Default `$PROJECT_ROOT/ralph/specs`.
- `RALPH_PLAN_FILE` : the ledger the loop marks off. Default `IMPLEMENTATION_PLAN.md`.

`/ralph-loop:run <name>` resolves the intent source before it starts the loop. An OpenSpec
change named `<name>` wins over `ralph/specs/<name>.md`. In OpenSpec mode it points
`RALPH_PLAN_FILE` at the change's own `tasks.md`, so a human using `/opsx:apply` and the
autonomous loop share one ledger with no synchronisation step.

`run` never invokes `/opsx:apply`. That command is human-in-loop and pauses to ask.

`rollback_all` preserves `$RALPH_PLAN_FILE` by relative path, because the ledger can sit
in a subdirectory. Changing that preservation logic risks losing checkbox state on every
rollback.
...
`ralph/gate_context.md`, `.diff_base`, or `ralph/specs/`. `scripts/schema_stamp.sh check`
warns when the installed copy is older than the plugin.
...
until files move, and routes the move to one command, chosen by whether OpenSpec is wired:
`/opsx:propose restructure-source` when it is, `/ralph-loop:spec restructure-source` when it
is not, then `/ralph-loop:run` either way. Never `/ralph-loop:slice`: that slices a story map
into one change per activity and stops without `ralph/AUDIENCE_JTBD.md`, and a restructure is
a single change. Moving working code is a refactor: it deserves a plan, a review and a
